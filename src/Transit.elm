...
{-| Transition step: Exit -> _send message_ -> Enter -> Done.
-}
type Step
    = Exit
    | Enter
...
tick tagger msg parent =
    let
        ...
        tag ( state_, fx ) =
            ( { parent | transition = T state_ }, Cmd.map tagger fx )
    in
    case msg of
...
subscriptions tagger parent =
    case getStep parent.transition of
        ...
        _ ->
            onAnimationFrame (posixToTime >> Tick >> tagger)
